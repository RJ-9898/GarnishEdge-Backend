...
from ..models import (creditor_debt_applied_rule,
                      creditor_debt_rule, creditor_debt_exempt_amt_config, creditor_debt_rule_edit_permission)
from django.core.exceptions import ValidationError
from GarnishEdge_Project.garnishment_library.utility_class import ResponseHelper
import logging
from ..serializers import (CreditorDebtAppliedRulesSerializers,
...
class CreditorDebtAppliedRuleAPIView(APIView):
    """
    API view to get applied creditor debt rule by case_id.
    """
    ...
    def get(self, request, case_id):
        try:
            rule = creditor_debt_applied_rule.objects.get(case_id=case_id)
            serializer = CreditorDebtAppliedRulesSerializers(rule)
            return ResponseHelper.success_response(
                f'Data for case_id "{case_id}" fetched successfully',
                serializer.data
            )
        except creditor_debt_applied_rule.DoesNotExist:
...
class CreditorDebtRuleAPIView(APIView):
    ...
    def get(self, request, state=None):
        try:
            if state:
                ...
                try:
                    rule = creditor_debt_rule.objects.get(
    ...
    def put(self, request, state=None):
        ...
        try:
            ...
            rule = creditor_debt_rule.objects.get(state__iexact=state)
        except creditor_debt_rule.DoesNotExist:
...
class CreditorDebtExemptAmtConfigAPIView(APIView):
    ...
    def get(self, request, pay_period=None, state=None):
        try:
            if state and pay_period:
                state = StateAbbreviations(
                    state.strip()).get_state_name_and_abbr()
                rule_qs = creditor_debt_exempt_amt_config.objects.filter(
                ...
                rules = creditor_debt_exempt_amt_config.objects.all()
                serializer = CreditorDebtExemptAmtConfigSerializers(
                    rules, many=True)
                return ResponseHelper.success_response(
    ...
    def put(self, request, pay_period=None, state=None):
        if not (state and pay_period):
            return ResponseHelper.error_response('State and pay_period are required in URL to update data', status_code=status.HTTP_400_BAD_REQUEST)
        try:
            rule = creditor_debt_exempt_amt_config.objects.get(
                state__iexact=state, pay_period__iexact=pay_period)
        except creditor_debt_exempt_amt_config.DoesNotExist:
        ...
        try:
            if state:
                try:
                    ...
                    rule = creditor_debt_rule_edit_permission.objects.get(
                        state__iexact=state)
                    serializer = CreditorDebtRuleEditPermissionSerializers(
                        rule)
                    return ResponseHelper.success_response(f'Data for state "{state}" fetched successfully', serializer.data)
                except creditor_debt_rule_edit_permission.DoesNotExist:
                    return ResponseHelper.error_response(f'State "{state}" not found', status_code=status.HTTP_404_NOT_FOUND)
            else:
                rules = creditor_debt_rule_edit_permission.objects.all()
                serializer = CreditorDebtRuleEditPermissionSerializers(
                    rules, many=True)
                return ResponseHelper.success_response('All data fetched successfully', serializer.data)
        except Exception as e:
